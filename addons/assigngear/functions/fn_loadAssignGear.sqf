...
if (_cfg isEqualTo missionConfigFile) then {
    _loadoutCode = [_faction, _role, _cfg] call FUNC(cacheAssignGear);
    GVAR(namespace) setVariable [_loadout, _loadoutCode];
    ...
    if (_hash isEqualTo {}) then {
        _loadoutCode = [_faction, _role, _cfg] call FUNC(cacheAssignGear);
        _loadoutsHash = [_loadoutsHash, _loadout, _loadoutCode] call CBA_fnc_hashSet;
