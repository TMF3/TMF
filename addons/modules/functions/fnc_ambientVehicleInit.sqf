...
switch _mode do {
    ...
    case "init": {
        ...
        if !(_isActivated) exitWith {
            ...
            private _count = 0;
            {
                ...
                if (alive _obj && (allPlayers findIf {
                    _x distance2D _obj < 500
                }) == -1) then {
                    deleteVehicle _obj;
        ...
        if (_area isEqualTo [] || {
            _area isEqualTo ([] call BIS_fnc_getArea)
        }) exitWith {
    ...
    case "preInit": {
        ...
        if is3DEN then {
            ...
            _syncedObjects = _connections apply {
                _x # 1
            };
            _area = (_syncedObjects select {
                _x isKindOf QEGVAR(ai, area)
            }) param [0, objNull];
        ...
        private _vehicleTypes = _syncedObjects apply {
            typeOf _x
        };
